...
R has rules about what constitutes a valid name. A syntactic name must contain letters, digits, `.` and `_` but cannot begin with a digit or underscore. There are also certain reserved names such as `TRUE` that cannot be used. A name that breaks these rules is non-syntactic. For details see [Non-syntactic names is Advanced R](https://adv-r.hadley.nz/names-values.html#non-syntactic).
...
[The tidyverse website](https://www.tidyverse.org/) has extensive materials for learning more about the tidyverse. And [Hands on Programming with R](https://rstudio-education.github.io/hopr/) provides a base R view.

## Tidy data {#sec-tidy-data}

Tidy data follows three principals for tabular data as proposed in the Tidy Data paper [@wickham2014]:
...
This is illustrated in @fig-tidy-data using a part of the Palmer Penguins dataset (@sec-palmer-penguins). The eight variables in the columns form an observation for each penguin in every row, and each cell has a single value recorded.
...
#| out-width: NULL
#| fig-cap: |
#|   Some tidy data from the penguin dataset. Every variable has it's own column, every observation has its own row and every value has its own cell.
#| fig-alt: |
#|  An image of tidy data using the penguin dataset. Every variable has it's own column, every observation has its own row and every value has its own cell.

knitr::include_graphics("img/tidy-data.png", dpi = 270)
```

Hadley Wickham explains in [R4DS tidy data](https://r4ds.hadley.nz/data-tidy):
...
This will hopefully become apparent in @sec-data-wrangling-i and @sec-data-wrangling-ii.
...
These are the building blocks of more complex data containers such as data frame (@sec-data-frames) containing the penguin dataset (@sec-palmer-penguins).
...
Note here that we see the use of the assignment operator `<-` to assign our vector on the right as the object `cards`. We talk more about that in @sec-assigning-objects.
...
In fact we could assign more than one label to the same object. Another way to think about this is that Bibi is a cat (@fig-bibi-object), and remains a cat even if I call her Pickle when she's being naughty.
...
[Colin Fay had a Twitter thread on this subject](https://twitter.com/_colinfay/status/1006139974377443328), but the reason I favour most is that it provides clarity. The arrow points in the direction of the assignment (it is actually possible to assign in the other direction too) and it distinguishes between creating an object in the workspace and assigning a value inside a function.
...
Don't worry if you find this confusing, everyone does when they first start with R. Hadley Wickham tweeted an image to illustrate list indexing shown in @fig-list-index.
...
Lists and `NULL` which indicates the absence of a vector, complete the set of base vectors in R as illustrated in @fig-r-vectors.
...
@fig-data-frame shows an example data frame we'll refer to as saved as the object `df` consisting of three rows and three columns. Each column is a different atomic data type of the same length.
...
Packages in the tidyverse create a modified form of data frame called a tibble. You can read about tibbles [here](http://r4ds.had.co.nz/tibbles.html).
...
<!-- and @sec-visualisation goes into detail -->
...
3.  Then pass some arguments to the aesthetics function `aes()` inside the `gpplot()` which tell ggplot how to plot the data e.g. which data goes on the x and y axis.
...
We'll use the `pengiuns` from the Palmer Penguins (@sec-palmer-penguins) dataset to the question *what is the relationship between body mass and flipper length?*
...
Check out the [ggplot2 documentation](http://ggplot2.tidyverse.org/reference/) for all the aesthetic possibilities (and Google for examples).
...
We'll spend more time on getting data in and out of our R environment in the @sec-data-wrangling-i, but let's imagine we wanted to export our plot and the `penguin` data for a colleague or presentation.
...
::: {.callout-tip .unnumbered appearance="simple" icon="true" title="`glue`  evaluation"}
We've not seen this syntax before, but [`glue`](https://glue.tidyverse.org/) is a tidyverse package for working with strings.
...
Here we are writing the `penguins` data frame as a csv file to the `outputs` folder and a file called `r glue("outputs/penguin-data-{Sys.Date()}.csv")`. The date in the filename is the date I built this book (`r Sys.Date()`)

### ggsave

If we want to save the last plot we made in `ggplot2` we can use the [`ggsave()`](https://ggplot2.tidyverse.org/reference/ggsave.html) function.
...
### Using `?` to access R function help pages {#sec-access-r-function-help-pages}
...
The introduction of [GPT-4](https://openai.com/research/gpt-4) and other powerful large language models such as [Claude 3](https://www.anthropic.com/) and [Gemini 1.5](https://deepmind.google/technologies/gemini/#introduction) since 2023 has meant that these tools are now some of the best ways to get help. with coding.

At the time of writing I'm still a novice in using them, but I highly recommend Ethan Mollick's work as a good place to start. He's written posts on [writing prompts](https://www.oneusefulthing.org/p/working-with-ai-two-paths-to-prompting) and has resources such as the [prompt library](https://www.moreusefulthings.com/prompts) for Open AI's ChatGPT.

I started with Claude 3 from Anthrophic. One reason I chose Claude is because of its [documentation](https://docs.anthropic.com/claude/docs/intro-to-claude) and [prompt library](https://docs.anthropic.com/claude/prompt-library).

For example, @fig-claude-1 shows how I prompted Claude 3 to help me reproduce the example I'd already created in @sec-filter-rows-with-filter.

1.  First I told Claude who I wanted it to be:
...
2.  Then I asked my new assistant Belle what I wanted:
...
#| out-width: 100%
#| fig-cap: |
#|   An example of using Claude 3 to create a R assistant that then showed me how to write the code to filter the Palmer Penguins data for bill length >= 50 mm.
#| fig-alt: |
#|  A screenshot of an example of using Claude 3 to create a R assistant that then showed me how to write the code to filter the Palmer Penguins data for bill length >= 50 mm.

knitr::include_graphics("img/claude-part-1-2024-04-04.png", dpi = 270)
```

Belle gave me a very good answer, but used the pipe operator `%>%` from `magrittr` rather than the native R pipe `|>` (@sec-the-pipe). So I asked Belle to change it. ("Can you" and "please" are probably redundant, but in my learning curve I've started by treating the models as people!)

``` markdown
Can you change this to use the native R pipe please?
```

Belle gets it perfect second time, @fig-claude-2.
...
#| out-width: 100%
#| fig-cap: |
#|   An example of updating my Claude 3 R assistants ouput to show me how to write the code to filter the Palmer Penguins data for bill length >= 50 mm using the native R pipe operator.
#| fig-alt: |
#|  A screenshot of an example of updating my Claude 3 R assistants ouput to show me how to write the code to filter the Palmer Penguins data for bill length >= 50 mm using the native R pipe operator.

knitr::include_graphics("img/claude-part-2-2024-04-04.png", dpi = 270)
```
...
### Using Google to find R answers {#sec-using-google-to-find-r-answers}
...
### Asking questions {#sec-asking-questions}
