...
To demonstrate the pivot as in @fig-pivot and @fig-pivot-animation, we need a unique
identifier for each row which I'll create  a variable called `id` using `mutatae` and the `dplyr` function `row_number()` which as you might guess, counts the row number and
returns an integer.
