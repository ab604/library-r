...
What is our usual trick for obtaining only the observations of interest? We `filter` the rows. But hang on, we can't do that unless we have the each affiliation in a row, rather spread across lots of columns. So what do?
...
In this way we create a table on which we can filter the rows for the UoS affiliations.
