...
I'll read it in directly from the repository for this book, and use `glimpse()`
to look at the contents. It has 483 rows and 20 columns, each row comprises a set of observations for a single publication as per @fig-tidy-data, the title of each publication is in the `Title` column.
...
So let's assume the question here is "Which University of Southampton Department(s) had author(s) on each publication?"

The departments are in the `Affiliations` column.
