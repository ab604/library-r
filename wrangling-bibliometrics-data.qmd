...
I'll read it in directly from the repository for this book

```{r}
scopus_dat <- read_csv("https://github.com/ab604/library-r/raw/main/data"
```


On opening up the spreadsheet I can see that it contains data regarding publications e.g. articles, books or reports. Each row comprises a set of observations for a single publication as per @fig-tidy-data, the title of each publication is in the `Title` column.

Imagine our question from the Export Control Team is: "Which University of Southampton

There are 643 rows and 35 columns. There are lots of empty cells i.e. missing values and I can see that column 14 is the `Affiliations` column.

If I extract the value of the `Affiliations` cell from the first row I can immediately see that it's not a single tidy value, it's a **messy** value with multiple values separated by commas like so:

```         
School of History and Philosophy of Science, The University of Sydney, Sydney, NSW, Australia; Centre for Philosophy of Natural and Social Science, London School of Economics and Political Science, London, United Kingdom; Department of Philosophy, University of Southampton, Avenue Campus, Southampton, United Kingdom
```

To be tidy, each comma separated value should be in their own column.

To make matters worse, each cell has a different number of values and the values are inconsistent either in the way a department is named or the information contained. Some cells have author names included for example.
