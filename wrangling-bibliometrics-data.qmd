...
1.  We need a function that separates the affiliations that are in one column into a columns for each affiliation.
2.  Different articles have different numbers of affiliations, so we will create missing values when we create new columns in the rows where no affiliation exists. We can fix that by changing the shape of the data with a pivot
