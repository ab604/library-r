...
Select `affiliations` and pipe it to `str_view()` so we can view all the values 
contained in the cell and how they are delimited.
...
scopus_lipid |> 
  select(affiliations) |> 
  mutate(affiliations = str_view(pattern = ";", affiliations)) |> 
  flextable() |> 
