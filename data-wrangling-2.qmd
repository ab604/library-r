...
[R4DS joins](https://r4ds.hadley.nz/joins) gives some detailed examples of joining
tables, but here we'll look at some simpler examples to try build our mental 
model.
...
2. Joins that *filter* the observations from one table against another, as per 
tidyverse verb `filter` @sec-filter-rows-with-filter.
