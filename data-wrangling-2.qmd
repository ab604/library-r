...
To explore `stringr` we'll create a couple of character vectors: Two character vectors are defined:
...
-   `>` is the literal character
-   `sample(c("-M_","-F_"), ...)` generates a random sex indicator (-M\_ for male or -F\_ for female) for each cat
